...
export const FormField: React.FC<FormFieldProps> = ({
  ...
  const inputClassName = `w-full bg-white/5 border ${
    hasError ? 'border-red-500' : 'border-white/10'
  } px-3 py-2 text-sm text-white placeholder-white/30 focus:outline-none focus:border-[#FF00A8] transition-all ${
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <label className={`text-[10px] font-mono uppercase tracking-wider ${hasError ? 'text-red-400' : 'text-white/40'}`}>
            {label}
          ...
          {tooltip && (
            <div className="group relative">
              ...
              <div className="absolute left-0 top-5 w-64 p-2 bg-black border border-white/20 text-[10px] text-white/80 leading-relaxed opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity z-50 shadow-xl">
                {tooltip}
              </div>
            </div>
          )}
